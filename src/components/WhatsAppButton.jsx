...
function WhatsAppButton() {
  const phoneNumber = "5493854997505";
  const message = "¡Hola! Quisiera saber más información sobre tus servicios.";
  ...
  useEffect(() => {
    ...
    // Entrada con elastic ease (similar a spring)
    gsap.fromTo(btn,
      { scale: 0, opacity: 0 },
      { scale: 1, opacity: 1, duration: 0.8, delay: 1.2, ease: "elastic.out(1, 0.5)" }
    );
    ...
    const onDown = () => gsap.to(btn, { scale: 0.9, duration: 0.1, ease: "power2.out" });
    const onUp = () => gsap.to(btn, { scale: 1.1, duration: 0.1, ease: "power2.out" });
